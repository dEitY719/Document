...
```
.
├── app.py                     # Dash 앱 인스턴스 및 최상위 레이아웃 정의
├── index.py                   # 다중 페이지 앱의 경우 콜백 및 라우팅
├── config/                    # 애플리케이션 전반의 설정 (환경 변수, 상수 등)
│   ├── __init__.py
│   └── settings.py
├── assets/                    # 정적 파일 (CSS, JS, 이미지, 로고 등)
│   ├── css/
│   ├── js/
│   └── images/
├── components/                # 재사용 가능한 UI 컴포넌트 모듈 (객체 지향적으로 구성)
│   ├── __init__.py
│   ├── layout/                # AppShell, Grid, Stack 등을 캡슐화한 레이아웃 컴포넌트
│   │   ├── __init__.py
│   │   ├── app_shell.py
│   │   └── responsive_grid.py
│   ├── data_display/          # Table, Image, Chart 등을 캡슐화한 컴포넌트
│   │   ├── __init__.py
│   │   ├── custom_table.py
│   │   └── dynamic_chart.py
│   ├── feedback/              # Alert, Loader 등을 캡슐화한 컴포넌트
│   │   ├── __init__.py
│   │   └── notification_banner.py
│   └── common/                # 범용적으로 사용될 UI 요소 (Button, Text 등)
│       └── custom_button.py
├── pages/                     # 다중 페이지 앱의 각 페이지 정의
│   ├── __init__.py
│   ├── home/
│   │   ├── __init__.py
│   │   ├── layout.py          # 페이지별 레이아웃 정의
...
│   │   └── callbacks.py
│   └── ...
├── themes/                    # Mantine 테마 정의
│   ├── __init__.py
│   └── main_theme.py          # dmc.MantineProvider의 theme 속성 정의
├── utils/                     # 유틸리티 함수 (데이터 처리, 헬퍼 함수 등)
│   ├── __init__.py
│   ├── data_utils.py
│   └── validation_utils.py
├── services/                  # 데이터 서비스 또는 API 호출 로직 (비즈니스 로직 분리)
│   ├── __init__.py
│   └── data_service.py
├── tests/                     # 테스트 코드 (TDD 준수)
│   ├── __init__.py
...
├── Dockerfile                 # (선택 사항) 배포를 위한 Docker 설정
├── requirements.txt           # 프로젝트 의존성
├── .env                       # 환경 변수 (예: 개발/운영 모드)
├── .pylintrc                  # Pylint 설정
├── pyproject.toml             # Mypy 설정 (PEP 518) 또는 기타 프로젝트 설정
└── README.md
```
